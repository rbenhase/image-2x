...
/**
 * @class RetinaImageModule
 */
class RetinaImageModule extends FLBuilderModule {
	...
	public function __construct()
	{
		parent::__construct(array(
			'name'          	=> __('Retina Image', 'fl-builder'),
			'description'   	=> __('Add a 2x (Retina) Image to your page.', 'fl-builder'),
	...
	public function get_data()
	{
		if(!$this->data) {
			...
			if($this->settings->photo_source == 'url') {
				...
				$this->data->link_2x = $this->settings->photo_url_2x;
				$this->data->url_2x = $this->settings->photo_url_2x;
				$this->settings->photo_src = $this->settings->photo_url;
				$this->settings->photo_src_2x = $this->settings->photo_url_2x;
			}
	...
	/**
	 * @method _get_cropped_path_2x
	 * @protected
	 */
	protected function _get_cropped_path_2x()
	{
		$crop        = empty($this->settings->crop) ? 'none' : $this->settings->crop;
		$url         = $this->_get_uncropped_url_2x();
		$cache_dir   = FLBuilderModel::get_cache_dir();
	...
	/**
	 * @method _get_uncropped_url_2x
	 * @protected
	 */
	protected function _get_uncropped_url_2x()
	{
		if($this->settings->photo_source == 'url') {
			$url = $this->settings->photo_url_2x;
		}
		else if(!empty($this->settings->photo_src)) {
			$url = $this->settings->photo_src;
		}
	...
	/**
	 * @method _get_cropped_demo_url_2x
	 * @protected
	 */
	protected function _get_cropped_demo_url_2x()
	{
		$info = $this->_get_cropped_path_2x();
 ...
 */
FLBuilder::register_module('RetinaImageModule', array(
	'general'       => array( // Tab
				...
				'fields'        => array( // Section Fields
					'photo_source'  => array(
						...
						'toggle'        => array(
							...
							'url'           => array(
								'fields'        => array('photo_url', 'photo_url_2x', 'caption')
							)
						)
					),
					'photo'         => array(
						'type'          => 'photo',
						'label'         => __('Regular (1x) Image', 'fl-builder'),
						'instructions'  => __('This will be used on regular (1x) screens, or as a fallback for browsers that lack srcset support.', 'fl-builder'),
					),
					'photo_2x'         => array(
						'type'          => 'photo',
						'label'         => __('Retina (2x) Image', 'fl-builder'),
						'instructions'  => __('This will be used on high density (e.g., Retina) screens.', 'fl-builder'),
					),
					'photo_url'     => array(
						'type'          => 'text',
						'label'         => __('Regular (1x) Image URL', 'fl-builder'),
						'placeholder'   => __( 'http://www.example.com/my-photo.jpg', 'fl-builder' ),
						'instructions'  => __('This will be used on regular (1x) screens, or as a fallback for browsers that lack srcset support.', 'fl-builder'),
					),
					'photo_url_2x'     => array(
						'type'          => 'text',
						'label'         => __('Retina (2x) Image URL', 'fl-builder'),
						'placeholder'   => __( 'http://www.example.com/my-photo.jpg', 'fl-builder' ),
						'instructions'  => __('This will be used on high density (e.g., Retina) screens.', 'fl-builder'),
					),
					'crop'          => array(
